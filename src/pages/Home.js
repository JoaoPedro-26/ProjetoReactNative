import React, { Component} from 'react';
import { Text, StyleSheet, View, TextInput, Button, Image, ScrollView, TouchableOpacity} from 'react-native';



export default class Home extends  Component{
    render() {
        return (
            <ScrollView style={styles.container}>
                <View style={styles.image_container}>
                    <Image style={styles.imagem} source={{uri: 'data:image/jpeg;base64,'}}/>
                </View>

                <View style={styles.login_form}>
                    <Text style={styles.title}>Acesse sua conta</Text>
                    <Text style={styles.titleInfos}>Email</Text>
                    <TextInput style={styles.login_input} placeholder='Digite seu email:' />
                    <Text style={styles.titleInfos}>Senha</Text>
                    <TextInput style={styles.login_input} placeholder='Digite sua senha:' secureTextEntry={true} />
                </View>

                <Button 
                    title="Acessar"
                    onPress={() => this.props.navigation.navigate('') }
                />

                <View style={styles.firstbutton}>
                    <TouchableOpacity style={styles.button} onPress={() => this.props.navigation.navigate('Login') }>
                        <Text style={styles.buttonText}>Ir para Login</Text>
                    </TouchableOpacity>
                </View>

                <View style={styles.firstbutton}>
                    <TouchableOpacity  style={styles.button} onPress={() => this.props.navigation.navigate('Infos') }>
                        <Text style={styles.buttonText}>Ir para Infos</Text>
                    </TouchableOpacity>
                </View>

                <View style={styles.firstbutton}>
                    <TouchableOpacity  style={styles.button} onPress={() => this.props.navigation.navigate('Lancamento') }>
                        <Text style={styles.buttonText}>Ir para Lançamento</Text>
                    </TouchableOpacity>
                </View>

            </ScrollView>
        )
    }
}

const styles = StyleSheet.create({
    container: {    
        backgroundColor: '#cccccc',
        height: '100%',
    },
    title: {
        fontSize: 30,
        marginBottom: 30,
    },
    titleInfos: {
        fontSize: 20,
        marginBottom: 5,
    },
    login_form: {
        width: "80%",
        marginHorizontal: 40,
        marginTop: 90,
    },
    login_input: {
        backgroundColor: "#fffafa",
        fontSize: 19,
        padding: 7,
        marginBottom: 15
    },
    imagem: {
        width: 120,
        height: 120,
        marginLeft: 130,
    },
    image_container: {
        opacity: 0.4,
        flexDirection: 'row',
    },
    firstbutton: {
        marginTop: 20,
        borderRadius: 10,
        alignItems: 'center',
    },
    button: {
        width: 200,
        height: 50,
        backgroundColor: '#fffafa',
        justifyContent: 'center',
        alignItems: 'center',
        borderRadius: 10,
        borderWidth: 3,
        borderColor: 'blue',
    },
    buttonText: {
        color: 'blue',
        fontSize: 20,
    }
});